#!/usr/bin/env bash
# Render site/untofu-icon.svg into the web icon set.
#
#   ./site/make-icons.sh
#
...
# device asks for.
sips -s format png -z 180 180 "$MASTER" --out "$HERE/untofu-apple-touch-icon.png" >/dev/null 2>&1
...
magick "$HERE/untofu-apple-touch-icon.png" +dither -colors 256 \
       -define png:compression-level=9 "$HERE/untofu-apple-touch-icon.png"

# Favicon carries 16/32/48 so each gets a real render rather than a browser-side
# downscale of one big one.
#
# The 16px slice comes from untofu-icon-small.svg instead. At that size the placeholder
...
qlmanage -t -s 1024 -o "$WORK" "$HERE/untofu-icon-small.svg" >/dev/null 2>&1
SMALL="$WORK/untofu-icon-small.svg.png"
[ -s "$SMALL" ] || { echo "✗ qlmanage did not render untofu-icon-small.svg" >&2; exit 1; }
...
magick "$WORK/f16.png" "$WORK/f32.png" "$WORK/f48.png" "$HERE/untofu-favicon.ico"
...
sips -s format jpeg -s formatOptions 82 -z 640 640 "$MASTER" \
     --out "$HERE/untofu-og.jpg" >/dev/null 2>&1

echo "✓ icon set:"
for f in untofu-icon.svg untofu-favicon.ico untofu-apple-touch-icon.png untofu-og.jpg; do
